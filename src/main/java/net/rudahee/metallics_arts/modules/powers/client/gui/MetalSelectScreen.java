package net.rudahee.metallics_arts.modules.powers.client.gui;
...
import net.rudahee.metallics_arts.modules.powers.client.PowersClientSetup;
...
public class MetalSelectScreen extends Screen {
    ...
    private static final String[] METAL_NAMES = Arrays.stream(MetalsNBTData.values()).map(MetalsNBTData::getNameLower).toArray(String[]::new);
    ...
    final Minecraft mc;
    int timeIn = PowerConfiguration.animate_selection.get() ? 0 : 16; // Config setting for whether the wheel animates open or instantly appears
    int slotSelected = -1;
    ...
    public void render(MatrixStack matrixStack, int mx, int my, float partialTicks) {
        ...
        this.mc.player.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(data -> {
            ...
            for (int seg = 0; seg < segments; seg++) {
                ...
                boolean mouseInSector = data.hasPower(metal) && (degPer * seg < angle && angle < degPer * (seg + 1));
    ...
    private void toggleSelected() {
        if (this.slotSelected != -1) {
            MetalsNBTData metal = MetalsNBTData.getMetal(toMetalIndex(this.slotSelected));
            this.mc.player.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(data -> {
                ClientUtils.toggleBurn(metal, data);
