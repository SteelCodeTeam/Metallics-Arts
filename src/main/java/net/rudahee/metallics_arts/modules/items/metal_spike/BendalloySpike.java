    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if ((target instanceof ServerPlayerEntity || target instanceof PlayerEntity) && (attacker instanceof ServerPlayerEntity || attacker instanceof PlayerEntity)){
            ...
            target.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(data ->{
                ...
                boolean hasAllomanticPower = super.hasAllomanticPower(MetalsNBTData.BENDALLOY, data);
                boolean hasFeruchemicPower = super.hasFeruchemicPower(MetalsNBTData.BENDALLOY, data);

                boolean couldStealPower = Math.random()>0.90;
                boolean couldRemovePower = Math.random()>0.50;
                boolean isAllomantic = Math.random()>0.50;

                if (!super.hasPlayerBothPowers(MetalsNBTData.BENDALLOY, data)) {
                    if (super.getFeruchemicNbt()){
                        data.addFeruchemicPower(MetalsNBTData.BENDALLOY);
                    } else if (super.getAllomanticNbt()){
                        data.addAllomanticPower(MetalsNBTData.BENDALLOY);
                    }
                } else {
                    if (super.hasPlayerBothPowers(MetalsNBTData.BENDALLOY, data)) {
                        if (isAllomantic) {
                            if (couldStealPower){
                                if (couldRemovePower){
                                    data.removeAllomanticPower(MetalsNBTData.BENDALLOY);
                                }
                            ...
                            if (couldStealPower){
                                if (couldRemovePower){
                                    data.removeFeruchemicPower(MetalsNBTData.BENDALLOY);
                                }
                                this.addItemToPlayer((PlayerEntity) target);
                            }
                        }
