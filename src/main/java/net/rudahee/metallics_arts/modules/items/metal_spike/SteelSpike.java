    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if ((target instanceof ServerPlayerEntity || target instanceof PlayerEntity) && (attacker instanceof ServerPlayerEntity || attacker instanceof PlayerEntity)){
            ...
            target.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(data ->{
                ...
                boolean hasAllomanticPower = super.hasAllomanticPower(MetalsNBTData.STEEL, data);
                boolean hasFeruchemicPower = super.hasFeruchemicPower(MetalsNBTData.STEEL, data);

                boolean couldStealPower = Math.random()>0.90;
                boolean couldRemovePower = Math.random()>0.50;
                boolean isAllomantic = Math.random()>0.50;

                if (!super.hasPlayerBothPowers(MetalsNBTData.STEEL, data)) {
                    if (super.getFeruchemicNbt()){
                        data.addFeruchemicPower(MetalsNBTData.STEEL);
                    } else if (super.getAllomanticNbt()){
                        data.addAllomanticPower(MetalsNBTData.STEEL);
                    }
                } else {
                    if (super.hasPlayerBothPowers(MetalsNBTData.STEEL, data)) {
                        if (isAllomantic) {
                            if (couldStealPower){
                                if (couldRemovePower){
                                    data.removeAllomanticPower(MetalsNBTData.STEEL);
                                }
                            ...
                            if (couldStealPower){
                                if (couldRemovePower){
                                    data.removeFeruchemicPower(MetalsNBTData.STEEL);
                                }
                                this.addItemToPlayer((PlayerEntity) target);
                            }
                        }
