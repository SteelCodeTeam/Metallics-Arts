...
public class LerasiumSpike extends MetalSpikeAbstract{

    public LerasiumSpike(Properties properties) {
        super(properties);
    }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if ((target instanceof ServerPlayerEntity || target instanceof PlayerEntity) && (attacker instanceof ServerPlayerEntity || attacker instanceof PlayerEntity)){
            ...
            target.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(targetCapability ->{
                ...
                boolean hasTargetAllomanticPower = super.hasPlayerAllomanticPower(MetalsNBTData.LERASIUM, targetCapability);
                boolean hasTargetFeruchemicPower = super.hasPlayerFeruchemicPower(MetalsNBTData.LERASIUM, targetCapability);
                ...
                if (super.getAllomanticNbt() || super.getFeruchemicNbt()) {
                    ...
                    if (super.getAllomanticNbt()) {
                        if (!super.hasPlayerAllomanticPower(MetalsNBTData.LERASIUM, targetCapability)) {
                            targetCapability.addAllomanticPower(MetalsNBTData.LERASIUM);
                        ...
                        if (!super.hasPlayerFeruchemicPower(MetalsNBTData.LERASIUM, targetCapability)) {
                            targetCapability.addFeruchemicPower(MetalsNBTData.LERASIUM);
                            target.hurt(DamageSource.MAGIC, 6);
                            new LightningBoltEntity(EntityType.LIGHTNING_BOLT, target.level).setVisualOnly(true);
                            target.level.playLocalSound(target.position().x, target.position().y, target.position().z, SoundEvents.GENERIC_EXPLODE ,SoundCategory.HOSTILE, 1.0f, 2.0f, true);
                    ...
                    if (super.hasPlayerBothPowers(MetalsNBTData.LERASIUM, targetCapability)) {
                        // 50% allomancy
                        if (isAllomantic) {
                            // 40% to steal
                            if (couldStealPower){
                                ...
                                if (couldRemovePower){
                                    targetCapability.removeAllomanticPower(MetalsNBTData.LERASIUM);
                                }
                            ...
                            if (couldStealPower){
                                // 50% of 40% to remove power
                                if (couldRemovePower){
                                    targetCapability.removeFeruchemicPower(MetalsNBTData.LERASIUM);
                                }
                                // Spike obtain the power
    ...
    public void addItemToPlayer(PlayerEntity attacker) {
        this.final_result = new ItemStack(ModItems.LERASIUM_SPIKE.get(), 1);
        final_result.addTagElement("lerasium_spike", super.getAllNbt());
