    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if ((target instanceof ServerPlayerEntity || target instanceof PlayerEntity) && (attacker instanceof ServerPlayerEntity || attacker instanceof PlayerEntity)){
            ...
            target.getCapability(InvestedCapability.PLAYER_CAP).ifPresent(targetCapability ->{
                ...
                boolean hasTargetAllomanticPower = super.hasPlayerAllomanticPower(MetalsNBTData.COPPER, targetCapability);
                boolean hasTargetFeruchemicPower = super.hasPlayerFeruchemicPower(MetalsNBTData.COPPER, targetCapability);
                ...
                if (super.getAllomanticNbt() || super.getFeruchemicNbt()) {
                    ...
                    if (super.getAllomanticNbt()) {
                        if (!super.hasPlayerAllomanticPower(MetalsNBTData.COPPER, targetCapability)) {
                            targetCapability.addAllomanticPower(MetalsNBTData.COPPER);
                        ...
                        if (!super.hasPlayerFeruchemicPower(MetalsNBTData.COPPER, targetCapability)) {
                            targetCapability.addFeruchemicPower(MetalsNBTData.COPPER);
                            target.hurt(DamageSource.MAGIC, 6);
                            new LightningBoltEntity(EntityType.LIGHTNING_BOLT, target.level).setVisualOnly(true);
                            target.level.playLocalSound(target.position().x, target.position().y, target.position().z, SoundEvents.GENERIC_EXPLODE ,SoundCategory.HOSTILE, 1.0f, 2.0f, true);
                    ...
                    if (super.hasPlayerBothPowers(MetalsNBTData.COPPER, targetCapability)) {
                        // 50% allomancy
                        if (isAllomantic) {
                            // 40% to steal
                            if (couldStealPower){
                                ...
                                if (couldRemovePower){
                                    targetCapability.removeAllomanticPower(MetalsNBTData.COPPER);
                                }
                            ...
                            if (couldStealPower){
                                // 50% of 40% to remove power
                                if (couldRemovePower){
                                    targetCapability.removeFeruchemicPower(MetalsNBTData.COPPER);
                                }
                                // Spike obtain the power
