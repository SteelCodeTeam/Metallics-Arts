...
/**
 * Class that specifies the atium and malatium ring, we pass to the abstract class (which has the behavior) the metals that compose it,
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> toolTips, TooltipFlag flagIn) {
        if (stack.hasTag()) {
            ...
            if (!Screen.hasControlDown()){
                ...
                toolTips.add(Component.translatable("metallics_arts.metal_translate."+getMetals(0).getNameLower()).append(": "+ ((stack.getTag().getInt(getMetals(0).getNameLower()+"_feruchemic_reserve") * 100)/this.getMetals(0).getMaxReserveRing())+"%"));
