...
public class EttmetalFence extends FenceBlock {

    public EttmetalFence(Properties properties) {
        super(properties);
    }
    ...
    public void tick(@NotNull BlockState blockState, @NotNull ServerLevel serverLevel, BlockPos blockPos, @NotNull RandomSource random) {
        if (serverLevel.isWaterAt(blockPos)) {
            serverLevel.explode(null, blockPos.getX(), blockPos.getY(), blockPos.getZ(), 8.0f, true, Level.ExplosionInteraction.BLOCK);
        }
