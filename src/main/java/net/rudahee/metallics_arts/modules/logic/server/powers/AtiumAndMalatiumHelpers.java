    ...
    private float internalDamageWhenUseAtium(Player source, Player target, float receivedQty) {
        ...
        source.getCapability(ModBlocksRegister.InvestedCapabilityRegister.PLAYER_CAP).ifPresent(sourceCap -> {
            ...
            target.getCapability(ModBlocksRegister.InvestedCapabilityRegister.PLAYER_CAP).ifPresent(targetCap -> {
                ...
                if (sourceCap.isBurning(MetalTagEnum.ATIUM)) {
                    if (sourceCap.isBurning(MetalTagEnum.LERASIUM) && !sourceCap.getEnhanced()) {
                        sourceState = 2;
                    } else if (sourceCap.getEnhanced() && !sourceCap.isBurning(MetalTagEnum.LERASIUM)) {
                        sourceState = 3;
                    } else if (sourceCap.isBurning(MetalTagEnum.LERASIUM) && sourceCap.getEnhanced()) {
                        sourceState = 4;
                    } else {
                        sourceState = 1;
                    }
                ...
                if (targetCap.isBurning(MetalTagEnum.ATIUM)) {
                    if (targetCap.isBurning(MetalTagEnum.LERASIUM) && !targetCap.getEnhanced()) {
                        targetState = 2;
                    } else if (targetCap.getEnhanced() && !targetCap.isBurning(MetalTagEnum.LERASIUM)) {
                        targetState = 3;
                    } else if (targetCap.isBurning(MetalTagEnum.LERASIUM) && targetCap.getEnhanced()) {
                        targetState = 4;
                    } else {
                        targetState = 1;
                    }
