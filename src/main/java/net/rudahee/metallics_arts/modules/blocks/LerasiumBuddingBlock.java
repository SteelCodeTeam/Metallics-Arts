    ...
    public void randomTick(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSourse) {
        if (randomSourse.nextInt(GROWTH_CHANCE) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = ModBlock.SMALL_LERASIUM_BUD.get();
            } else if (blockstate.is(ModBlock.SMALL_LERASIUM_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlock.MEDIUM_LERASIUM_BUD.get();
            } else if (blockstate.is(ModBlock.MEDIUM_LERASIUM_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlock.LARGE_LERASIUM_BUD.get();
            } else if (blockstate.is(ModBlock.LARGE_LERASIUM_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlock.LERASIUM_CLUSTER.get();
            }
